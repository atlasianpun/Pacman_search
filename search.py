...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not pq.isEmpty():
            ...
            for succState, succActions, succgCost in successors:
                updatedgCost = gCost + succgCost
                updatedfCost = updatedgCost + heuristic(succState, problem)
                # note: can also remove the below if statement - works the same
                if succState not in visited or updatedfCost < visited[succState]:
                    updatedActions = actions + [succActions]
                    pq.push((succState, updatedActions, updatedgCost), updatedfCost)
    return []
